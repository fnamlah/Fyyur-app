...
def show_venue(venue_id):
  ...
  for show in shows:
      ...
      arr_1 = re.split('-| - | 0', show_time)
      show_date = datetime(int(arr_1[0]), int(arr_1[1]), int(arr_1[2]))
      if show_date > today:
          upcoming_shows.append(show)
          print(f'Upcoming show: {show.artist.name}')
      else:
          past_shows.append(show)
          print(f'Past Show: {show.artist.name}')
